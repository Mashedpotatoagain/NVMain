...
AddOption('--cxx-standard', dest='cxx_standard', default='c++11',
          help='C++ standard to pass to the compiler, e.g. c++11, c++14, c++17')
AddOption('--disable-werror', dest='disable_werror', action='store_true',
          help='Do not promote compiler warnings to errors')
AddOption('--enable-werror', dest='enable_werror', action='store_true',
          help='Promote compiler warnings to errors')
...
#
gcc_like = any(tool in basename(os.environ.get('CXX', ''))
               for tool in ('g++', 'clang++', 'c++'))
if gcc_like:
...
cxx_standard = GetOption("cxx_standard")

common_flags = ['-Wall', '-Wextra', '-Woverloaded-virtual', '-fPIC',
                '-std=%s' % cxx_standard]
if GetOption("enable_werror") and not GetOption("disable_werror"):
    common_flags.append('-Werror')
...
#
env.Program(prog_name, src_list, PROGSUFFIX='.%s' % build_type)
